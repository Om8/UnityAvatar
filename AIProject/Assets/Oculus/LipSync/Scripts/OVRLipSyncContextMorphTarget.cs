...
public class OVRLipSyncContextMorphTarget : MonoBehaviour
{
    ...
    [Tooltip("Skinned Mesh Rendered target to be driven by Oculus Lipsync"), CannotBeNullObjectField]
	...
	public FacePositionList listOfMouthPositions;
    ...
    {
		if (listOfMouthPositions != null)
		{
			MouthPosition localMouthPosition = new MouthPosition(new float[50]);
			for (int i = 0; i < visemeToBlendTargets.Length; i++)
			{
				if (visemeToBlendTargets[i] != -1)
				{
					for (int x = 0; x < listOfMouthPositions.mouthPositions[i].blendShapes.Length; x++)
					{
						localMouthPosition.blendShapes[x] += listOfMouthPositions.mouthPositions[visemeToBlendTargets[i]].blendShapes[x] * frame.Visemes[i];
					}
				}
			}
			for (int y = 0; y < localMouthPosition.blendShapes.Length; y++)
			{
				// Viseme blend weights are assigned here.
				if (listOfMouthPositions.blendShapeList[y] == FaceRegion.Mouth)
				{
					skinnedMeshRenderer.SetBlendShapeWeight(y, localMouthPosition.blendShapes[y]);
				}
